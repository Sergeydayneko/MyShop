<?php
    ...
    $cat = clear_string($link,$_GET["cat"]); //вставляем ичищенные(защищенные от вирусов) теги и ссылки
    $type = clear_string($link,$_GET["type"]);

    $sorting = isset($_GET['sort']) ? $_GET['sort'] : null; //создаем сортировку
    switch ($sorting){

    case 'price-asc';
        $sorting = 'price ASC'; //asc - это снизу вверх, desc неаоборот
        $sort_name = 'Price up';
        break;
...
<div id="block-body">
    ...
    <div id="block-content">
            ...
            if (!empty($cat) && !empty($type)) { //проверяем что указываем товар и категория из БД

                $querycat = "AND brand = '$cat' AND product_type = '$type'"; //под соответствующий запрос формируем запрос в БД
                $catlink = "cat=$cat&"; //Под соответствующий запрос делается ссылка в url

            } else {

                if (!empty($type)) {
                    $querycat = "AND product_type = '$type'"; //В случае если только указан тип
                } else {
                    $querycat = " ";
                }
            ...
            $count = mysqli_query($link, "SELECT COUNT(*) FROM table_products WHERE visible = '1' $querycat"); //каунт подсчитывает все товары
            $temp = mysqli_fetch_array($count); //формирует количество товаров

            If ($temp[0] > 0) {
                $tempcount = $temp[0]; //помещаем количество товаров
                $total = (($tempcount - 1) / $num) + 1; //количество товаров делится на сколько надо выводить на 1 странице и поулчается общеее количетсво страниц
            ...
            <ul id="options-list">
                ...
                <li><a id="select-sort">' . $sort_name . '</a>
                    <ul id="sorting-list">
                        <li class="a"><a href="view_cat.php?' . $catlink . 'type=' . $type . '&page=1&sort=price-asc">Price up</a></li>
                        <li class="a"><a href="view_cat.php?' . $catlink . 'type=' . $type . '&page=1&sort=price-desc">Price down</a></li>
                        <li class="a"><a href="view_cat.php?' . $catlink . '&type=' . $type . '&page=1&sort=popular">Popular</a></li>
                        <li class="a"><a href="view_cat.php?' . $catlink . '&type=' . $type . '&page=1&sort=news">New products</a></li>
                        <li class="a"><a href="view_cat.php?' . $catlink . '&type=' . $type . '&page=1&sort=brand">From A to Z</a></li>
                    </ul>
                </li>
            </ul>
        </div>
        ...
        <ul id="block-product-list">
            ...
            if ($page - 5 > 0) { $page5left = '<li><a href="view_cat.php?' . $catlink . 'page=' . ($page - 5) . '&type=' . $type . '">' . ($page - 5) . '</a></li>'; } else $page5left = " ";
            if ($page - 4 > 0) { $page4left = '<li><a href="view_cat.php?' . $catlink . 'page=' . ($page - 4) . '&type=' . $type . '">' . ($page - 4) . '</a></li>'; } else $page4left = " ";
            if ($page - 3 > 0) { $page3left = '<li><a href="view_cat.php?' . $catlink . 'page=' . ($page - 3) . '&type=' . $type . '">' . ($page - 3) . '</a></li>'; } else $page3left = " ";
            if ($page - 2 > 0) { $page2left = '<li><a href="view_cat.php?' . $catlink . 'page=' . ($page - 2) . '&type=' . $type . '">' . ($page - 2) . '</a></li>'; } else $page2left = " ";
            if ($page - 1 > 0) { $page1left = '<li><a href="view_cat.php?' . $catlink . 'page=' . ($page - 1) . '&type=' . $type . '">' . ($page - 1) . '</a></li>'; } else $page1left = " ";

            //за правый вывод ссылок
            if ($page + 5 <= $total) { $page5right = '<li><a href="view_cat.php?' . $catlink . 'page=' . ($page + 5) . '&type=' . $type . '">' . ($page + 5) . '</a></li>'; } else $page5right = " ";
            if ($page + 4 <= $total) { $page4right = '<li><a href="view_cat.php?' . $catlink . 'page=' . ($page + 4) . '&type=' . $type . '">' . ($page + 4) . '</a></li>'; } else $page4right = " ";
            if ($page + 3 <= $total) { $page3right = '<li><a href="view_cat.php?' . $catlink . 'page=' . ($page + 3) . '&type=' . $type . '">' . ($page + 3) . '</a></li>'; } else $page3right = " ";
            if ($page + 2 <= $total) { $page2right = '<li><a href="view_cat.php?' . $catlink . 'page=' . ($page + 2) . '&type=' . $type . '">' . ($page + 2) . '</a></li>'; } else $page2right = " ";
            if ($page + 1 <= $total) { $page1right = '<li><a href="view_cat.php?' . $catlink . 'page=' . ($page + 1) . '&type=' . $type . '">' . ($page + 1) . '</a></li>'; } else $page1right = " ";
