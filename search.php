...
<head>
    ...
    <title>Searching... this shit <?php echo $search; ?> </title>
</head>
...
<div id="block-body">
    ...
    <div id="block-content">
        ...
        <?php
        if (strlen($search) >= 3 && strlen($search) < 150) { //вводим условие для поиска и оно распрострняется до коцна кода
        ?>
        ...
        <ul id="block-product-grid">
            ...
            if (!empty($_GET['page'])) {
                $page = (int)$_GET['page'];
            } else $page = ""; //помещение в переменную значение страницы

            $count = mysqli_query($link, "SELECT COUNT(*) FROM table_products WHERE title LIKE '%$search%' AND visible = '1'"); //вывод товаров у которых есть такое слово в название(в title)
            ...
            if ($temp[0] > 0) {
               //!!!!!!!!!!!!!!!!ТУТ ПОКАЗЫВАЕТСЯ НА СЕКУНДУ, ПОТОМ УБИРАЕТСЯ!!!!!!!!!!!!!!!!!!
                echo ' <div id="block-sorting">
            <p id="nav-breadcrums"><a href="">Main page</a> \ <span>Searching</span></p>
            <ul id="options-list">
                <li>View:</li>
    ...
    ';
                ...
                echo "<p>Didnt found any products</p>";
            }

            } else {
            echo "<p>Searching word must be from 3 to 150 symbols!</p>";
            }
            ?>﻿
